...
if(mysqli_num_rows($sql)>0){
    ...
    $cid=$row["cid"];
    $b["cid"]=$cid;
    $query2=mysqli_query($link,"select * from terminals where uid='".$uid."'") ;
    $row2=mysqli_fetch_assoc($query2);
    $bid = $row2["bid"];
    //echo "select * from cases where bid='".$bid."' order by patient_name ";
    $query=mysqli_query($link,"select * from terminals where bid='".$bid."' order by patient_name ") ;//to approve for Bloc Head
    $d=array();
    while($row1=mysqli_fetch_assoc($query))
    {
        $c=array();
        ...
        $c["approve"]=$row1["approve"]; 
        $uid33=$row1["update_uid"]; 
        $query3=mysqli_query($link,"select * from tterminal_users  where uid='".$uid33."'") ;
        $row3=mysqli_fetch_assoc($query3);
        $c["updated_by"] = $row3["phone"];

        $query2=mysqli_query($link,"select * from terminal_c where cid='".$row1["cid"]."'") ;
        $row2=mysqli_fetch_assoc($query2);
        $cname = $row2["cname"];
        $c["cname"]=$cname;    
        $query2=mysqli_query($link,"select * from terminals where bid='".$row1["bid"]."'") ;
        $row2=mysqli_fetch_assoc($query2);
        $cname = $row2["bname"];
        $c["bname"]=$cname;
        $query2=mysqli_query($link,"select * from panchayath where pid='".$row1["pid"]."'") ;
        ...
        $row2=mysqli_fetch_assoc($query2);
        $uid2 = $row2["uid"];
        $query3=mysqli_query($link,"select * from terminal_users where uid='".$uid2."'") ;
        $row3=mysqli_fetch_assoc($query3);
        $c["head_officer_name"] = $row3["officer_name"];
        $c["head_phone"] = $row3["phone"];
        array_push($d,$c);
